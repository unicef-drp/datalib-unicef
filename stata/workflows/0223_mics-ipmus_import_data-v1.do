*==================================================
*project:       D&A Datalib collection / SDG 2025 Report / IPUMS harmonization version 3 (August 2024)
...
global inputdata_ipums    "C:/Users/jpazevedo/UNICEF/MICS Harmonized Database - MICS Harmonized Data v1.1 [June 2023]/"
...
* convert WLD files from SAV to DTA (this step only needs to be done once)
    ...
    global inputdata_ipums    "C:\Users\jpazevedo\UNICEF\MICS Harmonized Database - MICS Harmonized Data v1.3 [August 2024]\"
...
/*
* This script processes survey files and organizes them into a structured folder system.
* The `overwrite` option determines whether existing `.dta` files will be replaced:
* - `local overwrite = 0`: Skip processing if the target `.dta` file already exists.
* - `local overwrite = 1`: Force processing and overwrite existing `.dta` files.
*
* To enable overwrite, set `local overwrite = 1`. Otherwise, leave it as 0.
*/

local overwrite = 0  // Set to 1 to overwrite existing files, 0 to skip them
        ...
        * create svy_id
        ...
        foreach id in `r(levels)' {
            ...
            di "`id'"
                ...
                * Define file paths
                local target_file "${datalib}/`ctry'/`ctry'_`year'_`svy'/`ctry'_`year'_`svy'_v01_M_v01_A_`clct'/Data/Stata/`ctry'_`year'_`svy'_v01_M_v01_A_`clct'_`type'.dta"

                * Check if target file already exists
                if fileexists("`target_file'") {
                    if `overwrite' == 0 {
                        di "File `target_file' already exists. Skipping."
                        continue
                    } 
                    else {
                        di "File `target_file' already exists. It will be overwritten."
                    }
                } 
                else {
                    di "File `target_file' does not exist. It will be created."
                }
                ...
                *create ctry folder if it does not exist
                cap: mkdir "${datalib}/`ctry'/`ctry'_`year'_`svy'/`ctry'_`year'_`svy'_v01_M_v01_A_`clct'"
                if (_rc==0) {
                    noi di "Created `ctry'_`year'_`svy' in datalib folder and subfolders"
                    cap: mkdir "${datalib}/`ctry'/`ctry'_`year'_`svy'/`ctry'_`year'_`svy'_v01_M_v01_A_`clct'/Doc"
                    cap: mkdir "${datalib}/`ctry'/`ctry'_`year'_`svy'/`ctry'_`year'_`svy'_v01_M_v01_A_`clct'/Data"
                    cap: mkdir "${datalib}/`ctry'/`ctry'_`year'_`svy'/`ctry'_`year'_`svy'_v01_M_v01_A_`clct'/Programs"
                    cap: mkdir "${datalib}/`ctry'/`ctry'_`year'_`svy'/`ctry'_`year'_`svy'_v01_M_v01_A_`clct'/Data/Original"
                    cap: mkdir "${datalib}/`ctry'/`ctry'_`year'_`svy'/`ctry'_`year'_`svy'_v01_M_v01_A_`clct'/Data/Stata"
                    cap: mkdir "${datalib}/`ctry'/`ctry'_`year'_`svy'/`ctry'_`year'_`svy'_v01_M_v01_A_`clct'/Data/Other"
                }   
                else {
                    noi di "`ctry'_`year'_`svy' in datalib folder already exists"
                }
                ...
                * save
                save "${datalib}/`ctry'/`ctry'_`year'_`svy'/`ctry'_`year'_`svy'_v01_M_v01_A_`clct'/Data/Stata/`ctry'_`year'_`svy'_v01_M_v01_A_`clct'_`type'.dta", replace
